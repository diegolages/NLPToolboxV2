...
public final class Real extends Number<Real> implements Field<Real> {
    ...
    public static Real valueOf(double doubleValue) {
        ...
        int e = MathLib.floorLog10(Math.abs(doubleValue)) - 18 + 1; // 18 digits significand.
    ...
    public Real plus(Real that) {
        if ((this == NaN) || (that == NaN))
            return NaN;
        //if (Math.abs(this._exponent) > Math.abs(that._exponent))
    ...
    public Real times(Real that) {
        ...
        real._significand = this._significand.times(that._significand);

 ...
 * @author Diego
 */
public class Secant<F extends Field<F>> implements LineSearch<F> {
    ...
    public F minimize(LinearFunction<F> f, F initialx ) throws Exception {
        ...
        F xn = initialx;
        F xn_1 = initialx.times(numberfactory.makeNumber(2));
        ...
        do {
           ...
           // xn = xn.plus( f.evaluate(xn).times(  f.derivative(xn).inverse() ).opposite()  );
            ...
            F part = xn.plus(xn_1.opposite()).times(f.evaluate(xn).plus(f.evaluate(xn_1).opposite()));

            xn_1 = xn;

            xn = xn.plus(part.times(f.evaluate(xn)).opposite());
    ...
    public static void main(String argv[]) throws Exception {
        ...
        //Secant
        Secant nr = new Secant();
        nr.setMaxiterations(10000000);
        nr.setPrecision(Float64.valueOf(0.000001));

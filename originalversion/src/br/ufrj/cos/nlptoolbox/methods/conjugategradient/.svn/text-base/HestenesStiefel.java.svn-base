 ...
 * @author Diego
 */
public class HestenesStiefel<F extends Field<F>> extends ConjugateGradient<F> implements MinimizationMethod<F> {
    ...
    protected F getBeta(Vector<F> deltaxn, Vector<F> deltaxn_1,Vector<F> lambdaxn_1,Function<F> function, Vector<F> xn) {
        ...
        return deltaxn.times(yn).times(  lambdaxn_1.times(yn).inverse()    );

...
package br.ufrj.cos.nlptoolbox.gui;
...
import br.ufrj.cos.nlptoolbox.linesearch.Armijo;
import br.ufrj.cos.nlptoolbox.linesearch.LineSearch;
import br.ufrj.cos.nlptoolbox.linesearch.NewtonRaphson;
import br.ufrj.cos.nlptoolbox.linesearch.Secant;
import br.ufrj.cos.nlptoolbox.linesearch.Wolfe;
...
public class MainJPanel extends javax.swing.JPanel {
    ...
    private void initComponents() {
        ...
        functionTextField.setText("x1^2 + x2^2");
...
private void exitButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_exitButtonActionPerformed
    ...
    private NumberFactory getNumberFactory() {
        if ("Rational".equals(numerictypeComboBox.getSelectedItem())) {
            return new RationalNumberFactory();
        } else if ("Real".equals(numerictypeComboBox.getSelectedItem())) {
            return new RealNumberFactory();
        } else if ("FloatingPoint".equals(numerictypeComboBox.getSelectedItem())) {
            return new FloatingPointNumberFactory();
        } else if ("Float64".equals(numerictypeComboBox.getSelectedItem())) {
            return new Float64NumberFactory();
        } else if ("Complex".equals(numerictypeComboBox.getSelectedItem())) {
...
private void processButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_processButtonActionPerformed
    ...
    NumberFactory numberfactory = getNumberFactory();
    ...
    if (!restrictions.isEmpty()) {
        ...
        for (String r : restrictions) {
            ExpressionFunction ef = new ExpressionFunction(r);
            ef.setNumberfactory(numberfactory);
...
//        method3.setInitialc(numberfactory.makeNumber(Double.parseDouble(jInitialc.getText())));
   ...
   // NewtonRaphson linesearch = new NewtonRaphson();
    ...
    linesearch.setPrecision(numberfactory.makeNumber(0.000001));

    method.setLinesearch(linesearch);
        
    
    ExpressionFunction ef = new ExpressionFunction(functionTextField.getText());
    ef.setNumberfactory(numberfactory);

    method.setMaxiterations(Integer.parseInt(maxiterationsTextField.getText()));
    method.setNorm(new QuadraticNorm());
    method.setParameters(numberfactory.makeNumber(Double.parseDouble(alphaTextField.getText())));
    method.setPrecision(numberfactory.makeNumber(Double.parseDouble(precisionTextField.getText())));

    java.util.List ipelements = new java.util.Vector();

    String ipstring = initialpointTextField.getText();
    String ss[] = ipstring.split(",");
    for (String s : ss) {
        ipelements.add(numberfactory.makeNumber(Double.parseDouble(s)));
    }

    Vector initialpoint = DenseVector.valueOf(ipelements);
...
// TODO add your handling code here:
    try {
        ...
        ExpressionFunction ef = new ExpressionFunction(functionTextField.getText());
        ef.setNumberfactory(new Float64NumberFactory());
